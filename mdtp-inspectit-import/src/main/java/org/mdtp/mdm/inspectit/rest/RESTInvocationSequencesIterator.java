package org.mdtp.mdm.inspectit.rest;
...
public class RESTInvocationSequencesIterator implements Iterator<InvocationSequenceData> {
	...
	private static final String OVERVIEW_PATH = "/rest/data/invocations/overview";
	private static final String DETAILS_PATH = "/rest/data/invocations/details";
	...
	private void fetchNextIds() {
		...
		try {
			result = rest.performGet(OVERVIEW_PATH,InvocationSequenceData[].class,params);
		} catch (IOException e) {
	...
	public InvocationSequenceData next() {
		...
		try {
			return rest.performGet(DETAILS_PATH,InvocationSequenceData.class,params);
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
